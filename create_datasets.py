...
def create_trump_dataset(trump_path):
...
def create_movie_dataset(movie_path):
  outfolder = os.path.join(CLIPPED_VIDEO_FOLDER, 'hitch_hiker')
  video = VideoFileClip(movie_path)
  video.duration # we have this length, split it up into smaller segments, which can also be split up into different frame # offsets
